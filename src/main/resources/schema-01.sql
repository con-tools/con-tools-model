...
CREATE TABLE IF NOT EXISTS `user_records` (
  ...
  PRIMARY KEY (`id`) COMMENT '',
  FOREIGN KEY `fk_user_record_user_id` (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE,
  FOREIGN KEY `fk_user_record_convention_id` (`convention_id`) REFERENCES `conventions` (`id`) ON DELETE CASCADE
) ENGINE=INNODB CHARACTER SET UTF8;
...
CREATE TABLE IF NOT EXISTS `convention_records` (
  ...
  `data` MEDIUMBLOB NOT NULL COMMENT 'schemaless data, format depends on content_type',
  PRIMARY KEY (`id`) COMMENT '',
  FOREIGN KEY `fk_con_record_convention_id` (`convention_id`) REFERENCES `conventions` (`id`) ON DELETE CASCADE
) ENGINE=INNODB CHARACTER SET UTF8;
